...
def run_inference():
    ...
    try:
        ...
        if stage == "stage1":
            ...
            # --- 数据路径 ---
            opt.inference_image_full_path = os.path.join(
                base_dir, "lr_preprocessed", "cropped_volume_FULL.h5"
            )
            opt.path_image_hr = os.path.join(
            ...
            opt.voxel_size_hr = 2.4
            opt.inference_out_name = "3D_SR"
            opt.inference_checkpoint_epoch = target_epoch         # ← 改好了
            opt.inference_plane = None                            # yz / xz 时再覆盖
            ...
            # --- 网络/训练超参（保持与训练期一致即可） ---
            opt.crop_size           = default_opt["crop_size"]
            opt.num_crops           = default_opt["num_crops"]
            opt.gen_lr              = default_opt["gen_lr"]
            opt.disc_lr             = default_opt["disc_lr"]
            opt.w_cycle_loss        = default_opt["w_cycle_loss"]
            opt.w_identity_loss     = default_opt["w_identity_loss"]
            opt.train_batch_size    = default_opt["train_batch_size"]
            opt.test_batch_size     = default_opt["test_batch_size"]
            opt.gen_numResBlocks    = default_opt["gen_numResBlocks"]
            opt.disc_numInterBlocks = default_opt["disc_numInterBlocks"]
            opt.lr_image_channels   = 1
            opt.hr_image_channels   = 1
            opt.test_rate           = 1000
            opt.gen_baseFilters = 64
            ...
            # --- 输入（来自 Stage-1 输出） ---
            opt.create_training_testing_dataset = False
            opt.path_image_yz = os.path.join(base_dir, "Inference", "Stage1", "3D_SR_yz.h5")
            opt.path_image_xz = os.path.join(base_dir, "Inference", "Stage1", "3D_SR_xz.h5")

            # --- 网络/数据尺寸 ---
            opt.crop_size           = default_opt["crop_size"]          # 96
            opt.num_crops           = default_opt["num_crops"]
            opt.gen_in_channels     = 1
            opt.gen_baseFilters     = 64
            opt.gen_numResBlocks    = default_opt["gen_numResBlocks"]   # 4
            opt.disc_baseFilters    = 64
            opt.disc_numInterBlocks = default_opt["disc_numInterBlocks"]
            ...
            # --- 输出 ---
            opt.inference_out_name         = "3D_Final"
            opt.inference_checkpoint_epoch = checkpoint_epoch or default_opt["n_epochs"]
            opt.test_rate                  = 1000
...
@auth_bp.route('/history-list', methods=['GET'])
@jwt_required()
def history_list():
    UPLOAD_ROOT = current_app.config['UPLOAD_ROOT']
    ...
    user_root = os.path.join(UPLOAD_ROOT, user['username'])
    # 列出所有 history_* 文件夹并排序
    histories = sorted(
        [d for d in os.listdir(user_root) if d.startswith('history_')],
        key=lambda x: int(x.split('_')[1])
    )
    return jsonify({'histories': histories}), 200


@auth_bp.route('/list-checkpoints', methods=['GET'])
@jwt_required()
def list_checkpoints():
    ...
    username = user['username']
    # 如果没传，或者格式不对，就自动选最新那个 history_xxx
    history_id = request.args.get('history_id', '').strip()
    print("history_id",history_id)
    if not re.match(r'^history_\d+$', history_id):
        # 列出所有 history_* 文件夹
        user_root = os.path.join(UPLOAD_ROOT, username)
        all_histories = [d for d in os.listdir(user_root)
                         if os.path.isdir(os.path.join(user_root, d)) and d.startswith('history_')]
        if not all_histories:
            return jsonify({'checkpoints': []}), 200
        # 排序后取最新
        all_histories.sort(key=lambda x: int(x.split('_')[1]))
        history_id = all_histories[-1]

    # 构造 Stage2 checkpoints 文件夹路径
    checkpoint_dir = os.path.join(UPLOAD_ROOT, username, history_id, 'checkpoints', 'Stage2')
    print("checkpoint_dir",checkpoint_dir)

    if not os.path.exists(checkpoint_dir):
        print("Not the checkpoint path")
        return jsonify({'checkpoints': []}), 200

    # 获取所有 .pt 文件名
    checkpoint_files = sorted([
        f for f in os.listdir(checkpoint_dir)
        if os.path.isfile(os.path.join(checkpoint_dir, f)) and f.endswith('.pt')
    ])
    print("ck files",checkpoint_files)

    return jsonify({'checkpoints': checkpoint_files}), 200


# ================== download Stage-2 result ==================
@auth_bp.route("/download-stage2-result")
@jwt_required()
def dl_stage2():
    history_id = request.args.get("history_id")
    ...
    if not user:
        return jsonify({"error": "User not found"}), 404
    if not history_id or not fname:
        return jsonify({"error": "missing param"}), 400

    # ---------- 正确的文件路径 ----------
    # uploads_root/<username>/<history_id>/Inference/Stage2/3D_Final.h5|tif
    path = os.path.join(
    ...
    # attachment; filename*=utf-8''... 兼容中文
    headers = {
        "Content-Disposition": f"attachment; filename*=UTF-8''{fname}"
    }

    return current_app.response_class(generate(), mimetype=mime, headers=headers)

@auth_bp.route("/download-stage2-zip")
@jwt_required()
def download_stage2_zip():
    ...
    if not user:
        return jsonify({"error": "User not found"}), 404
    if not history_id or not re.match(r"^history_\d+$", history_id):
        return jsonify({"error": "Invalid or missing history_id"}), 400
    ...
    h5_path  = os.path.join(base_dir, "3D_Final.h5")
    tif_path = os.path.join(base_dir, "3D_Final.tif")

    missing = [p for p in [h5_path, tif_path] if not os.path.exists(p)]
    if missing:
        return jsonify({"error": f"Missing file(s): {', '.join(os.path.basename(x) for x in missing)}"}), 404
    ...
    with ZipFile(tmp_name, "w", ZIP_DEFLATED) as zf:
        zf.write(h5_path, arcname="3D_Final.h5")
        zf.write(tif_path, arcname="3D_Final.tif")

    # send_file 会自动带 Content-Length，前端可显示进度
    return send_file(
        tmp_name,
        mimetype="application/zip",
        as_attachment=True,
        download_name=f"{history_id}_stage2_results.zip",
        max_age=0,
        conditional=True,
    )
